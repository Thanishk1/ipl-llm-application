...
# ---- MAIN ----
schedule_url_2025 = "https://www.espncricinfo.com/series/ipl-2025-1449924/match-schedule-fixtures-and-results"
base_urls_2025 = get_all_match_base_urls(schedule_url_2025, "ipl-2025-1449924")

print(f"Found {len(base_urls_2025)} completed matches for IPL 2025")

all_reports = []
for i, url in enumerate(base_urls_2025):
    try:
        report = scrape_match_report(url)
        all_reports.append(report)
        print(f"[{i+1}/{len(base_urls_2025)}] Scraped: {report['title']}")
    except Exception as e:
        print(f"[{i+1}/{len(base_urls_2025)}] FAILED: {url} — {e}")
    time.sleep(1.5)  # be polite, avoid hammering the server

# Save incrementally
with open("ipl_2025_reports.json", "w", encoding="utf-8") as f:
